...
mod tests {
    ...
    fn test_to_u16() -> Result<()> {
        ...
        for i in 1..U16_LENGTH_BOUND {
            ...
            let x: &[u8] = &x_vec[..];
            let val = biscuit.to_u16(x).unwrap();
            assert_eq!(
    ...
    fn test_u16_max() -> Result<()> {
        ...
        let val = biscuit.to_u16(max_byte);
        assert_eq!(val, Some(u16::MAX));
        ...
        let val_p1 = biscuit.to_u16(x_p1);
        let val_n1 = biscuit.to_u16(x_n1);
        assert_eq!(val_p1, None);
        assert_eq!(val_n1, Some(u16::MAX - 1));
        Ok(())
    }
    ...
    fn test_u16_leading_zeros() -> Result<()> {
        ...
        let val_leading_zeros = biscuit.to_u16(x_leading_zeros);
        assert_eq!(val_leading_zeros, Some(12345));
        Ok(())
    }
}